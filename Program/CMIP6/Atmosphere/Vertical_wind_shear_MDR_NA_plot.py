...
for model_i in range(len(models)):
	...
	filename_UVEL		= directory+models[model_i]+'/Atmosphere/Vertical_wind_shear_MDR_NA.nc'
...
for period in ['PRESENT', 'FUTURE']:
	...
	for ensemble_i in range(len(ensemble_number)):
		...
		time_rcp, u_vel = ReadinDataMDR(directory_UH_CESM+'_'+str(ensemble_number[ensemble_i]).zfill(3)+'/Atmosphere/Vertical_wind_shear_MDR_NA.nc', month_start, month_end)
...
time_ERA5, u_vel_ERA5			= ReadinDataMDR(directory_ERA5+'/Atmosphere/Vertical_wind_shear_MDR_NA_1993-2017.nc', month_start, month_end)
time_rcp_high, u_vel_rcp_high	= ReadinDataMDR(directory_HR_CESM+'/Atmosphere/Vertical_wind_shear_MDR_NA.nc', month_start, month_end)
...
ax.set_title('e) MDR North Atlantic, June - November')
