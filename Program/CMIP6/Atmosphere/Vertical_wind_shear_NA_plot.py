...
fh = netcdf.Dataset(directory_ERA5+'Atmosphere/Vertical_wind_shear_NA_month_'+str(month_start)+'-'+str(month_end)+'_'+str(year_start_ERA)+'-'+str(year_end_ERA)+'.nc', 'r')
...
for period in ['PRESENT', 'FUTURE']:
	...
	for model_i in range(len(models)):
		...
		filename_UVEL		= directory+models[model_i]+'/Atmosphere/Vertical_wind_shear_NA_month_'+str(month_start)+'-'+str(month_end)+'_year_'+str(year_start).zfill(3)+'-'+str(year_end).zfill(3)+'.nc'
...
ax.set_title('a) Zonal vertical wind shear, $\Delta$CMIP6, June - November')
