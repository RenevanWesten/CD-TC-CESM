...
TEMP_data = netcdf.Dataset(directory+'Atmosphere/GPI_index_PRESENT_NA_month_'+str(month_start)+'-'+str(month_end)+'.nc', 'r')

months			= TEMP_data.variables['month'][:]
lat				= TEMP_data.variables['lat'][:] 			
lon				= TEMP_data.variables['lon'][:] 			
RH_present		= TEMP_data.variables['RH'][:] 			
VWS_present		= TEMP_data.variables['VWS'][:] 			
abs_vor_present	= TEMP_data.variables['ABS_VOR'][:] 			
v_pot_present	= TEMP_data.variables['MPI'][:] 
area			= TEMP_data.variables['AREA'][:] 
...
TEMP_data = netcdf.Dataset(directory+'Atmosphere/GPI_index_FUTURE_NA_month_'+str(month_start)+'-'+str(month_end)+'.nc', 'r')
			
RH_future		= TEMP_data.variables['RH'][:] 			
VWS_future		= TEMP_data.variables['VWS'][:] 			
abs_vor_future	= TEMP_data.variables['ABS_VOR'][:] 			
v_pot_future	= TEMP_data.variables['MPI'][:] 

TEMP_data.close()

#-----------------------------------------------------------------------------------------

GPI_present			= np.abs(abs_vor_present * 10**5.0)**(3.0/2.0) * (RH_present / 50.0)**3.0 * (v_pot_present / 70.0)**3.0 * (1.0 + 0.1 * VWS_present)**(-2.0)
GPI_future			= np.abs(abs_vor_future * 10**5.0)**(3.0/2.0) * (RH_future / 50.0)**3.0 * (v_pot_future / 70.0)**3.0 * (1.0 + 0.1 * VWS_future)**(-2.0)
...
#Determine the GPI where only one variable is altered
GPI_future_vor		= np.abs(abs_vor_future * 10**5.0)**(3.0/2.0) * (RH_present / 50.0)**3.0 * (v_pot_present / 70.0)**3.0 * (1.0 + 0.1 * VWS_present)**(-2.0)
GPI_future_RH		= np.abs(abs_vor_present * 10**5.0)**(3.0/2.0) * (RH_future / 50.0)**3.0 * (v_pot_present / 70.0)**3.0 * (1.0 + 0.1 * VWS_present)**(-2.0)
GPI_future_v_pot	= np.abs(abs_vor_present * 10**5.0)**(3.0/2.0) * (RH_present / 50.0)**3.0 * (v_pot_future / 70.0)**3.0 * (1.0 + 0.1 * VWS_present)**(-2.0)
GPI_future_VWS		= np.abs(abs_vor_present * 10**5.0)**(3.0/2.0) * (RH_present / 50.0)**3.0 * (v_pot_present / 70.0)**3.0 * (1.0 + 0.1 * VWS_future)**(-2.0)
...
ax.set_title('g) GPI MDR, North Atlantic') 
...
ax.set_title('e) 700 hPa Relative humidity, $\Delta$UH-CESM')
