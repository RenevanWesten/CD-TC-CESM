...
lon_max_index		= (fabs(lon - 180)).argmin() + 1
...
ax2.text(np.mean(lat_mean_present) - 1.0, 0.048, str(round(np.mean(lat_mean_present), 1)), verticalalignment='top', horizontalalignment='right', color = 'k', fontsize=14)
ax2.text(np.mean(lat_mean_future) + 1, 0.048, str(round(np.mean(lat_mean_future), 1)), verticalalignment='top', horizontalalignment='left', color = 'r', fontsize=14)
...
ax.set_title('d) ITCZ latitude, May - November')
show()
