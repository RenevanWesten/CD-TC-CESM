...
TOA_present		= ma.masked_all((5, 322, 256))
TOA_future		= ma.masked_all((5, 322, 256))
LW_present		= ma.masked_all((5, 322, 256))
LW_future		= ma.masked_all((5, 322, 256))
SW_present		= ma.masked_all((5, 322, 256))
SW_future		= ma.masked_all((5, 322, 256))
LH_present		= ma.masked_all((5, 322, 256))
LH_future		= ma.masked_all((5, 322, 256))
SH_present		= ma.masked_all((5, 322, 256))
SH_future		= ma.masked_all((5, 322, 256))
SHF_present		= ma.masked_all((5, 322, 256))
SHF_future		= ma.masked_all((5, 322, 256))
#-----------------------------------------------------------------------------------------
...
for period in ['PRESENT', 'FUTURE']:
	...
	for ensemble_i in range(len(ensemble_number)):
		...
		print(ensemble_i)
		fh = netcdf.Dataset(directory+'_'+str(ensemble_number[ensemble_i]).zfill(3)+'/Atmosphere/TOA_SHF_SW_LW_NA_month_'+str(month_start)+'-'+str(month_end)+'.nc', 'r')
...
ax.set_title('c) North Atlantic energy budget, $\Delta$UH-CESM')
#-----------------------------------------------------------------------------------------
...
ax.set_title('f) North Atlantic energy budget, $\Delta$UH-CESM')
