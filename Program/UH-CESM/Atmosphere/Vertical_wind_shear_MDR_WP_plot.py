...
for period in ['PRESENT', 'FUTURE']:
	...
	for ensemble_i in range(len(ensemble_number)):
		...
		fh = netcdf.Dataset(directory+'_'+str(ensemble_number[ensemble_i]).zfill(3)+'/Atmosphere/Vertical_wind_shear_MDR_WP.nc', 'r')
...
#-----------------------------------------------------------------------------------------
fh = netcdf.Dataset(directory_ERA5+'/Atmosphere/Vertical_wind_shear_MDR_WP_1993-2017.nc', 'r')
...
legend        = ax.legend(graphs, legend_labels, loc = 'upper left', ncol=1, fancybox=True, numpoints = 1)
...
ax.set_title('f) Zonal vertical wind shear climatology')
...
plot([4.5, 10.5], [-4.5, -4.5], '-k', linewidth = 2.0)
plot([4.5, 4.5], [-5, -4], '-k', linewidth = 2.0)
plot([10.5, 10.5], [-5, -4], '-k', linewidth = 2.0)
ax.text(4.4, -4.5, 'May', verticalalignment='center', horizontalalignment='right', fontsize=10)
...
ax.set_title('h) Zonal vertical wind shear climatology difference')
