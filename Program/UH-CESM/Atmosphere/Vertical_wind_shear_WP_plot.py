...
u_vel_present	= ma.masked_all((5, 280, 355))
u_vel_future	= ma.masked_all((5, 280, 355))
...
for period in ['PRESENT', 'FUTURE']:
	...
	for ensemble_i in range(len(ensemble_number)):
		...
		print(ensemble_i)
		fh 		= netcdf.Dataset(directory+'_'+str(ensemble_number[ensemble_i]).zfill(3)+'/Atmosphere/Vertical_wind_shear_WP_month_'+str(month_start)+'-'+str(month_end)+'.nc', 'r')
...
ax.set_title('d) UH-CESM$^{\mathrm{PD}}$, May - November')
...
ax.set_title('b) Zonal vertical wind shear, May - November')
...
ax.set_title('b) Zonal vertical wind shear, $\Delta$UH-CESM, May - November')
