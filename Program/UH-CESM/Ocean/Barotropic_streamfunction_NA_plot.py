#Program plots the barotropic streamfunction and wind stress curl
...
BSF_present			= ma.masked_all((5, 960, 1101))
BSF_future			= ma.masked_all((5, 960, 1101))
WIND_present		= ma.masked_all((5, 962, 1103))
WIND_future			= ma.masked_all((5, 962, 1103))
...
for period in ['PRESENT', 'FUTURE']:
	...
	for ensemble_i in range(len(ensemble_number)):
		...
		print(ensemble_i)
		fh 		= netcdf.Dataset(directory+'_'+str(ensemble_number[ensemble_i]).zfill(3)+'/Ocean/Barotropic_streamfunction_NA_month_'+str(month_start)+'-'+str(month_end)+'.nc', 'r')
		...
		fh 		= netcdf.Dataset(directory+'_'+str(ensemble_number[ensemble_i]).zfill(3)+'/Ocean/Wind_stress_curl_NA_month_'+str(month_start)+'-'+str(month_end)+'.nc', 'r')
		...
		fh = netcdf.Dataset(directory+'_'+str(ensemble_number[ensemble_i]).zfill(3)+'/Ocean/Wind_stress_curl_Atlantic_basin_month_'+str(month_start)+'-'+str(month_end)+'.nc', 'r')
...
ax.set_title('a) Barotropic streamfunction and wind stress curl, UH-CESM$^{\mathrm{PD}}$')
...
ax.set_title('c) Barotropic streamfunction and wind stress curl, $\Delta$UH-CESM')
show()
#-----------------------------------------------------------------------------------------
