#Program plots the globally-averaged sea surface temperature, including spin-up and HR-CESM
...
time_HR, temp_HR	= ReadinData(directory_HR_CESM+'Ocean/SST_global.nc')
...
for period in ['PRESENT', 'FUTURE']:
	...
	if period == 'PRESENT':
		...
		time_UH_spin, temp_UH_spin	= ReadinData(directory_UH_CESM+'/Ocean/SST_global_spin_up_year_2002.nc')
	...
	if period == 'FUTURE':
		...
		#Get the spin up for the UH-CESM
		time_UH_spin, temp_UH_spin	= ReadinData(directory_UH_CESM+'/Ocean/SST_global_spin_up_year_2092.nc')

	for ensemble_i in range(len(ensemble_number)):
		#Loop over each ensemble
		time, temp	= ReadinData(directory_UH_CESM+'_'+str(ensemble_number[ensemble_i]).zfill(3)+'/Ocean/SST_global.nc')
...
ax.set_ylabel('Global mean sea surface temperature ($^{\circ}$C)')
ax.set_ylim(18, 22)
ax.grid()
